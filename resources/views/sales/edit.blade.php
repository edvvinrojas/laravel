...
<div class="card">
    <div class="card-body grid grid-cols-1 md:grid-cols-2 gap-4">
        @php
            $availableItems = $items->filter(fn($i) => $i->location_status === 'BODEGA' || $i->id === $sale->item_id);
            $unavailableItems = $items->reject(fn($i) => $i->location_status === 'BODEGA' || $i->id === $sale->item_id);
        @endphp
        <div>
        ...
        <div class="md:col-span-2 pt-2">
            ...
            <input type="hidden" name="item_id" id="item_id_input" value="{{ old('item_id', $sale->item_id) }}" required>
                    ...
                    @foreach($availableItems as $i)
                        ...
                        >
                            ...
                            <div class="mt-2 flex items-center justify-between">
                                <span class="text-[11px] px-2 py-1 rounded {{ $i->id === $sale->item_id && $i->location_status !== 'BODEGA' ? 'bg-amber-100 text-amber-700' : 'bg-gray-100 text-gray-600' }}">
                                    {{ $i->id === $sale->item_id && $i->location_status !== 'BODEGA' ? 'ACTUAL: '.$i->location_status : ($i->location_status ?? 'BODEGA') }}
                                </span>
