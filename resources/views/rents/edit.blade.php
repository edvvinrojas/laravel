...
<div class="card">
    ...
    <div class="card-body grid grid-cols-1 md:grid-cols-2 gap-4">
        @php
            $selectedIds = $rent->items->pluck('id');
            if ($selectedIds->isEmpty() && $rent->item_id) {
                $selectedIds = collect([$rent->item_id]);
            }
...
@php
    ...
    if ($initialItemRows === null) {
        $initialItemRows = $rent->items->map(function ($item) use ($rent) {
            return [
                'item_id' => $item->id,
                ...
                'bn_included' => $item->pivot->bn_included ?? $rent->bn_included,
                'bn_cost_per_excess' => $item->pivot->bn_cost_per_excess ?? $rent->bn_cost_per_excess,
                'color_included' => $item->pivot->color_included ?? $rent->color_included,
                'color_cost_per_excess' => $item->pivot->color_cost_per_excess ?? $rent->color_cost_per_excess,
            ];
        })->values()->all();
    }
@endphp
...
const initialRows = @json($initialItemRows);
