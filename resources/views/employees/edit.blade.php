...
<div class="card">
    <div class="card-body grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="col-span-2"><label class="form-label">Jefe directo</label><select name="direct_manager_user_id" class="form-select"><option value="">Sin asignar</option>@foreach($managers as $m)<option value="{{ $m->id }}" @selected(old('direct_manager_user_id',$employee->direct_manager_user_id)==$m->id)>{{ $m->full_name }} ({{ strtoupper($m->department) }})</option>@endforeach</select></div>
