...
<div class="card">
    <div class="card-body space-y-4">
        <div>
            ...
            <input name="name" type="text" value="{{ old('name', $serviceType->name) }}" class="form-input" required>
            @error('name')<p class="form-error">{{ $message }}</p>@enderror
        </div>
        <div>
            <label class="form-label">Descripción</label>
            <textarea name="description" class="form-input" rows="2">{{ old('description', $serviceType->description) }}</textarea>
        </div>
        <div class="flex items-center gap-2">
            <input type="checkbox" name="is_active" id="is_active" value="1" class="form-checkbox"
                   @checked(old('is_active', $serviceType->is_active))>
