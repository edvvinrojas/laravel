...
@section('page-title','Editar Accesorio')

@section('content')
<div class="max-w-lg">
<form method="POST" action="{{ route('accesorios.update', $accesorio) }}" class="space-y-4">
@csrf @method('PUT')
<div class="card">
    ...
    <div class="card-body space-y-4">
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input name="nombre" value="{{ old('nombre', $accesorio->nombre) }}"
            ...
            <div>
                ...
                <input name="codigo" value="{{ old('codigo', $accesorio->codigo) }}"
        ...
        <div>
            ...
            <input type="number" name="precio" value="{{ old('precio', $accesorio->precio) }}"
                   class="form-input" min="0" step="0.01">
        </div>
        <div>
            <label class="form-label">Descripción</label>
            <textarea name="descripcion" rows="2" class="form-input">{{ old('descripcion', $accesorio->descripcion) }}</textarea>
        </div>
        <div class="flex items-center gap-2">
            <input type="checkbox" id="es_activo" name="es_activo" value="1"
                   {{ old('es_activo', $accesorio->es_activo) ? 'checked' : '' }}
