...
@section('page-title','Editar '.$quote->quote_number)
...
<div class="card mb-5">
    <div class="card-header">
        ...
        <span class="font-mono text-xs text-blue-700 bg-blue-50 border border-blue-200 rounded px-2 py-0.5">{{ $quote->quote_number }}</span>
    </div>
    <div class="card-body grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
            ...
            <input type="date" name="valid_until" value="{{ $quote->valid_until?->format('Y-m-d') }}" class="form-input">
        </div>

        <div class="md:col-span-3">
            <label class="form-label">Notas / Condiciones</label>
            <textarea name="notes" rows="2" class="form-input">{{ $quote->notes }}</textarea>
        </div>
    </div>
</div>
...
<div class="card mb-5">
    <div class="card-header">
        <h3 class="font-semibold text-sm">Agregar productos del catálogo</h3>
    </div>
    <div class="card-body">
        ...
        <div class="cat-tab-content" data-tab="equipos">
            <div class="h-48 overflow-y-auto pr-1">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    ...
                    <button type="button" class="catalog-card h-24 flex flex-col justify-between text-left border border-gray-200 rounded-lg p-3 transition {{ $available ? 'hover:border-blue-300 hover:bg-blue-50/40' : 'opacity-50 cursor-not-allowed bg-gray-50' }}"
                        data-type="item" data-id="{{ $i->id }}"
                        data-label="{{ trim(($i->brand->name ?? '').' '.$i->model) }}"
                        data-price="{{ $i->cost ?? 0 }}" {{ !$available ? 'disabled' : '' }}>
        ...
        <div class="cat-tab-content hidden" data-tab="refacciones">
            <div class="h-48 overflow-y-auto pr-1">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    ...
                    <button type="button" class="catalog-card h-24 flex flex-col justify-between text-left border border-gray-200 rounded-lg p-3 hover:border-green-300 hover:bg-green-50/40 transition"
                        data-type="sparepart" data-id="{{ $sp->id }}"
                    ...
                    @empty
                    <div class="col-span-3 py-6 text-center text-sm text-gray-400">Sin refacciones</div>
                    @endforelse
                </div>
            </div>
        </div>
        ...
        <div class="cat-tab-content hidden" data-tab="inventario">
            <div class="h-48 overflow-y-auto pr-1">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    ...
                    <button type="button" class="catalog-card h-24 flex flex-col justify-between text-left border border-gray-200 rounded-lg p-3 hover:border-purple-300 hover:bg-purple-50/40 transition"
                        data-type="inventory" data-id="{{ $inv->id }}"
                    ...
                    @empty
                    <div class="col-span-3 py-6 text-center text-sm text-gray-400">Sin artículos</div>
                    @endforelse
                </div>
            </div>
        </div>
        ...
        <div class="mt-2">
            <button type="button" onclick="addManualLine()" class="btn-secondary text-xs">+ Agregar línea manual</button>
        </div>
    </div>
</div>
    ...
    <div class="card-body p-0">
        <div class="table-wrap rounded-none border-0">
            <table class="table">
                ...
                <tbody id="linesTbody">
                    <tr id="emptyRow" style="{{ $quote->lines->count() ? 'display:none' : '' }}">
                        <td colspan="6" class="text-center py-8 text-gray-400 text-sm">Sin líneas</td>
                    </tr>
                </tbody>
                <tfoot>
                    <tr class="bg-gray-50">
...
<div class="flex justify-end gap-3">
    <a href="{{ route('quotes.show', $quote) }}" class="btn-secondary">Cancelar</a>
...
document.querySelectorAll('.cat-tab').forEach(tab => {
    tab.addEventListener('click', function () {
        ...
        document.querySelector(`.cat-tab-content[data-tab="${this.dataset.tab}"]`).classList.remove('hidden');
    });
});
...
function recalc() {
    ...
    linesTbody.querySelectorAll('tr[data-line]').forEach(row => {
        ...
        const lt = qty * price;
        total += lt;
        row.querySelector('.line-total').textContent = formatMoney(lt);
    });
...
function addLine(type, id, label, price, quantity) {
    const idx = lineIndex++;
    ...
    tr.innerHTML = `
        ...
        <td>
            <input type="number" name="lines[${idx}][quantity]" value="${qty}" min="1" required
        ...
        <td class="text-right font-semibold text-sm line-total pr-4">${formatMoney(qty * price)}</td>
...
function addManualLine() { addLine('manual', null, '', 0, 1); }

function removeLine(btn) {
    btn.closest('tr[data-line]').remove();
    recalc();
    linesTbody.querySelectorAll('tr[data-line]').forEach((row, i) => { row.cells[0].textContent = i + 1; });
}

document.querySelectorAll('.catalog-card:not([disabled])').forEach(card => {
    card.addEventListener('click', function () {
        addLine(this.dataset.type, this.dataset.id, this.dataset.label, parseFloat(this.dataset.price) || 0, 1);
    });
});

// Cargar líneas existentes
existingLines.forEach(l => addLine(l.type, l.id, l.description, l.unit_price, l.quantity));

document.getElementById('quoteForm').addEventListener('submit', function (e) {
    if (linesTbody.querySelectorAll('tr[data-line]').length === 0) {
        e.preventDefault();
