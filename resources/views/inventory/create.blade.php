...
<div class="card">
    <div class="card-body grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
            ...
            <input name="item_code" value="{{ old('item_code') }}" class="form-input" required>
        </div>
        <div>
            ...
            <select name="catalog_id" class="form-select" required>
                ...
                <option value="{{ $c->id }}" @selected(old('catalog_id')==$c->id)>{{ $c->item_name }} — {{ $c->item_type }}</option>
                @endforeach
            </select>
        </div>
        <div>
            ...
            <select name="shelf_id" class="form-select">
                ...
                <option value="{{ $s->id }}" @selected(old('shelf_id')==$s->id)>{{ $s->name }}</option>
                @endforeach
            </select>
        </div>
        <div>
        ...
        <div>
            ...
            <input name="invoice" value="{{ old('invoice') }}" class="form-input">
        </div>
        <div>
            <label class="form-label">Fecha de entrada</label>
            <input name="entry_date" type="date" value="{{ old('entry_date') }}" class="form-input">
        </div>
        <div>
            <label class="form-label">Costo</label>
            <input name="cost" type="number" step="0.01" min="0" value="{{ old('cost') }}" class="form-input">
        </div>
        <div class="flex items-center gap-2 mt-5">
            <input name="is_available" type="checkbox" id="is_available" value="1" class="form-checkbox" @checked(old('is_available',1))>
    ...
    <div class="px-5 py-4 border-t border-gray-100 flex gap-3">
        ...
        <a href="{{ route('inventory.index') }}" class="btn-secondary">Cancelar</a>
    </div>
</div>
</form>
</div>
