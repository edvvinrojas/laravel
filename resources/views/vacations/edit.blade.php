...
<div class="card">
    <div class="card-body grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div><label class="form-label">Días restantes *</label><input name="remaining_days" type="number" min="0" value="{{ old('remaining_days',$vacation->remaining_days) }}" class="form-input" required></div>
        ...
        <div><label class="form-label">Estado *</label><select name="status" class="form-select" required>@foreach(['PENDIENTE','APROBADO','RECHAZADO','ACTIVO','PAGADO'] as $s)<option value="{{ $s }}" @selected(old('status',$vacation->status)===$s)>{{ $s }}</option>@endforeach</select></div>
