...
<div class="card">
    <div class="card-body grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div><label class="form-label">Monto pendiente *</label><input name="pending_amount" type="number" step="0.01" min="0" value="{{ old('pending_amount') }}" class="form-input" required></div>
        <div><label class="form-label">Quincenas pendientes *</label><input name="pending_biweeks" type="number" min="0" value="{{ old('pending_biweeks') }}" class="form-input" required></div>
        <div><label class="form-label">Fecha de aprobación</label><input name="approval_date" type="date" value="{{ old('approval_date') }}" class="form-input"></div>
        ...
        <div><label class="form-label">Estado *</label><select name="status" class="form-select" required>@foreach(['SOLICITADO','AUTORIZADO','LIQUIDADO','CANCELADO'] as $s)<option value="{{ $s }}" @selected(old('status','SOLICITADO')===$s)>{{ $s }}</option>@endforeach</select></div>
